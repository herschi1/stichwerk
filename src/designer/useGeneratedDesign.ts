...
const EMPTY: GeneratedDesign = { stitches: [], blockColors: [], width: 0, height: 0 };

/** Regenerates the stitches (debounced) whenever the design changes. */
export function useGeneratedDesign(elements: DesignElement[]) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        const result = generateStitches(elements, fonts);
        if (!cancelled) {
          setDesign(result);
          setError(null);
        }
